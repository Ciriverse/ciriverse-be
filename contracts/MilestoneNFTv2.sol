...
contract MilestoneNFTv2 is ReentrancyGuard,ERC1155 {
            ...
            for (uint256 i = 0; i < milestoneCount; i++) {
                ...
                if (
                    (s_donators[msg.sender][creator].fund >=
                        s_milestoneArray[creator][i].price) && s_donators[msg.sender][creator].minted[i] == false
                ) {
    ...
    function addMilestone(
        ...
        internal
        isRegistered(msg.sender)
        // isOwner(nftAddress, tokenId, creator)
    {
    ...
    function isCreator(address creatorAddress) internal view returns (bool) {
        Creator memory creator = s_creators[creatorAddress];
        if (creator.addr == address(0)) {
            return false;
        }
    ...
    function isEligibleToMint(address creatorAddress, address donator, uint256 milestoneId)
        public
        view
        returns (bool)
    {
        if (
            s_donators[donator][creatorAddress].status[milestoneId] == true && s_donators[donator][creatorAddress].minted[milestoneId] == false
        ) {
            return true;
        }
        return false;
